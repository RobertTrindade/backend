...
import { AccountStatus, Usuarios } from "@prisma/client";
...
class Service {
  ...
  public async loginAdm(data: LoginDto) {
    // find admin.
    const admin = await this.findByCredential(data.email);
  ...
  public async forgotPasswordAdm(data: ForgotPasswordDto) {
    ...
    await MailService.sendForgotPasswordEmail(admin.email,admin.name, { code, minutes });
    return { message: "Código de recuperação de senha enviado no seu email!" };
  }

  public async resetPasswordAdm(data: ResetPasswordDto) {
    // find admin.
    const admin = await this.findByCredentialAndCode(
      data.credential,
      data.code
    );
  ...
  private checkIfAdminIsActive(admin: Usuarios) {
